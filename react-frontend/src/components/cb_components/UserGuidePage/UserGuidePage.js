...
import UserGuideDatatable from "./UserGuideDataTable";
...
const UserGuidePage = (props) => {
  ...
  const deleteRow = async () => {
    try {
      await client.service("userGuide").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) => client.service("userGuide").remove(e._id));
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
          props.alert({
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="userGuide-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-11" role="userGuide-datatable">
          <UserGuideDatatable
            items={data}
            fields={fields}
...
export default connect(mapState, mapDispatch)(UserGuidePage);
