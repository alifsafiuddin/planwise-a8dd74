...
const CompanyAddressesPage = (props) => {
  ...
  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("companyAddresses");
  ...
  const deleteRow = async () => {
    try {
      await client.service("companyAddresses").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("companyAddresses").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  return (
    <div className="mt-5">
      ...
      <div className="grid align-items-center">
        <div className="col-11" role="companyAddresses-datatable">
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <CompanyAddressesEditDialogComponent
