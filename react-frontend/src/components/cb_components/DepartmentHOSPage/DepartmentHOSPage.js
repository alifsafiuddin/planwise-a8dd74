...
import DepartmentHOSDatatable from "./DepartmentHOSDataTable";
...
import DepartmentHOSFakerDialogComponent from "./DepartmentHOSFakerDialogComponent";
import DepartmentHOSSeederDialogComponent from "./DepartmentHOSSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const DepartmentHOSPage = (props) => {
  ...
  const filename = "departmentHOS.csv";
  ...
  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("departmentHOS");
  ...
  const deleteRow = async () => {
    try {
      await client.service("departmentHOS").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("departmentHOS").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  const onRowClick = ({ data }) => {
    navigate(`/departmentHOS/${data._id}`);
  };
  ...
  return (
    <div className="mt-5">
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="departmentHOS-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-11" role="departmentHOS-datatable">
          <DepartmentHOSDatatable
            items={data}
            fields={fields}
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <DepartmentHOSEditDialogComponent
...
export default connect(mapState, mapDispatch)(DepartmentHOSPage);
