...
import PositionsCreateDialogComponent from "./PositionsCreateDialogComponent";
import PositionsFakerDialogComponent from "./PositionsFakerDialogComponent";
import PositionsSeederDialogComponent from "./PositionsSeederDialogComponent";
...
const PositionsPage = (props) => {
  ...
  const deleteRow = async () => {
    try {
      await client.service("positions").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) => client.service("positions").remove(e._id));
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
          props.alert({
  ...
  return (
    <div className="mt-5">
      ...
      <div className="grid align-items-center">
        <div className="col-11" role="positions-datatable">
          <PositionsDatatable
            items={data}
            fields={fields}
...
export default connect(mapState, mapDispatch)(PositionsPage);
