...
const StaffinfoPage = (props) => {
  ...
  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("staffinfo");
  ...
  const deleteRow = async () => {
    try {
      await client.service("staffinfo").remove(selectedEntityIndex);
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="staffinfo-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-11" role="staffinfo-datatable">
          <StaffinfoDatatable
            items={data}
            fields={fields}
