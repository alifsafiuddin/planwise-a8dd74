...
import DepartmentAdminDatatable from "./DepartmentAdminDataTable";
...
import DepartmentAdminFakerDialogComponent from "./DepartmentAdminFakerDialogComponent";
import DepartmentAdminSeederDialogComponent from "./DepartmentAdminSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const DepartmentAdminPage = (props) => {
  ...
  const filename = "departmentAdmin.csv";
  ...
  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("departmentAdmin");
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          departmentId: urlParams.singleDepartmentsId,
  ...
  const deleteRow = async () => {
    try {
      await client.service("departmentAdmin").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("departmentAdmin").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  const onRowClick = ({ data }) => {
    navigate(`/departmentAdmin/${data._id}`);
  };
  ...
  return (
    <div className="mt-5">
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="departmentAdmin-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-11" role="departmentAdmin-datatable">
          <DepartmentAdminDatatable
            items={data}
            fields={fields}
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <DepartmentAdminEditDialogComponent
...
export default connect(mapState, mapDispatch)(DepartmentAdminPage);
