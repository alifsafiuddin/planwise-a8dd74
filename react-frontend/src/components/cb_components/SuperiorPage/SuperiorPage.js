...
import SuperiorDatatable from "./SuperiorDataTable";
...
import SuperiorFakerDialogComponent from "./SuperiorFakerDialogComponent";
import SuperiorSeederDialogComponent from "./SuperiorSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const SuperiorPage = (props) => {
  ...
  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("superior");
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          superior: urlParams.singleStaffinfoId,
          subordinate: urlParams.singleStaffinfoId,
          $populate: [
            {
              path: "createdBy",
              service: "users",
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "Superior",
          type: "error",
          message: error.message || "Failed get Superior",
        });
      });
  ...
  const deleteRow = async () => {
    try {
      await client.service("superior").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) => client.service("superior").remove(e._id));
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
          props.alert({
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="superior-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-11" role="superior-datatable">
          <SuperiorDatatable
            items={data}
            fields={fields}
...
export default connect(mapState, mapDispatch)(SuperiorPage);
