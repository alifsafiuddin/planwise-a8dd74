...
import incomesFakerFactory from "./incomesFakerFactory";
...
const IncomesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = incomesFakerFactory(props.user,fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("incomes").create(elem));
...
export default connect(mapState, mapDispatch)(IncomesFakerDialogComponent);
