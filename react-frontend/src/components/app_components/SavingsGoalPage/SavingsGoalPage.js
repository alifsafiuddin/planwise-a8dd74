...
import SavingsGoalDatatable from "./SavingsGoalDataTable";
import SavingsGoalEditDialogComponent from "./SavingsGoalEditDialogComponent";
import SavingsGoalCreateDialogComponent from "./SavingsGoalCreateDialogComponent";
import SavingsGoalFakerDialogComponent from "./SavingsGoalFakerDialogComponent";
import SavingsGoalSeederDialogComponent from "./SavingsGoalSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const SavingsGoalPage = (props) => {
    ...
    const filename = "savingsGoal.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("savingsGoal");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "SavingsGoal", type: "error", message: error.message || "Failed get SavingsGoal" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("savingsGoal").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "SavingsGoal", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("savingsGoal").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="savingsGoal-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="savingsGoal-datatable">
                    <SavingsGoalDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <SavingsGoalEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <SavingsGoalCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <SavingsGoalFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <SavingsGoalSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(SavingsGoalPage);
