...
import savingsGoalFakerFactory from "./savingsGoalFakerFactory";
...
const SavingsGoalFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = savingsGoalFakerFactory(props.user,fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("savingsGoal").create(elem));
...
export default connect(mapState, mapDispatch)(SavingsGoalFakerDialogComponent);
