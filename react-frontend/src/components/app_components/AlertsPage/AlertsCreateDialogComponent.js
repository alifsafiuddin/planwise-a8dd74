...
const AlertsCreateDialogComponent = (props) => {
  ...
  const onSave = async () => {
    ...
    try {
      const result = await client.service("alerts").create(_data);
      const eagerResult = await client.service("alerts").find({
        query: {
          $limit: 10000,
          _id: { $in: [result._id] },
          $populate: [
      ...
      props.alert({
        type: "success",
        title: "Create info",
        message: "Info Alerts updated successfully",
      });
      ...
      props.alert({
        type: "error",
        title: "Create",
        message: "Failed to create in Alerts",
      });
    }
    setLoading(false);
  };
  ...
  return (
    ...
    >
      <div
        ...
        style={{ maxWidth: "55vw" }}
        role="alerts-create-dialog-component"
      >
        ...
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <InputText
              ...
              value={_entity?.message}
              onChange={(e) => setValByKey("message", e.target.value)}
              required
            />
          </span>
          <small className="p-error">
