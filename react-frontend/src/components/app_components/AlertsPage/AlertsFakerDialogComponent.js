...
import alertsFakerFactory from "./alertsFakerFactory";
...
const AlertsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = alertsFakerFactory(props.user,fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("alerts").create(elem));
...
export default connect(mapState, mapDispatch)(AlertsFakerDialogComponent);
