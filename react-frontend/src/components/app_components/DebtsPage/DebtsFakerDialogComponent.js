...
import debtsFakerFactory from "./debtsFakerFactory";
...
const DebtsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = debtsFakerFactory(props.user,fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("debts").create(elem));
...
export default connect(mapState, mapDispatch)(DebtsFakerDialogComponent);
