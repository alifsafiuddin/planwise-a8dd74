...
const ExpensesCreateDialogComponent = (props) => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const eagerResult = await client.service("expenses").find({
        query: {
          $limit: 10000,
          _id: { $in: [result._id] },
          $populate: [
      ...
      props.alert({
        type: "success",
        title: "Create info",
        message: "Info Expenses updated successfully",
      });
      ...
      props.alert({
        type: "error",
        title: "Create",
        message: "Failed to create in Expenses",
      });
    }
    setLoading(false);
  };
  ...
  return (
    ...
    >
      <div
        ...
        style={{ maxWidth: "55vw" }}
        role="expenses-create-dialog-component"
      >
        ...
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <Calendar
              id="dateSpent"
              value={_entity?.dateSpent ? new Date(_entity?.dateSpent) : null}
              dateFormat="dd/mm/yy"
              onChange={(e) =>
                setValByKey("dateSpent", new Date(e.target.value))
              }
              showIcon
              showButtonBar
            />
