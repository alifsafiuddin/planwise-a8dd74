...
import expensesFakerFactory from "./expensesFakerFactory";
...
const ExpensesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = expensesFakerFactory(props.user,fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("expenses").create(elem));
...
export default connect(mapState, mapDispatch)(ExpensesFakerDialogComponent);
