...
import AiFinancialAdviceDatatable from "./AiFinancialAdviceDataTable";
...
import AiFinancialAdviceFakerDialogComponent from "./AiFinancialAdviceFakerDialogComponent";
import AiFinancialAdviceSeederDialogComponent from "./AiFinancialAdviceSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const AiFinancialAdvicePage = (props) => {
    ...
    const filename = "aiFinancialAdvice.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("aiFinancialAdvice");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , userId : urlParams.singleUsersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "AiFinancialAdvice", type: "error", message: error.message || "Failed get AiFinancialAdvice" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("aiFinancialAdvice").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "AiFinancialAdvice", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("aiFinancialAdvice").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "AiFinancialAdvice",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/aiFinancialAdvice/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>AiFinancialAdvice </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="aiFinancialAdvice-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="aiFinancialAdvice-datatable">
                    <AiFinancialAdviceDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <AiFinancialAdviceEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <AiFinancialAdviceCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <AiFinancialAdviceFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <AiFinancialAdviceSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(AiFinancialAdvicePage);
