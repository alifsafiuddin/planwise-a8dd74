...
import aiFinancialAdviceFakerFactory from "./aiFinancialAdviceFakerFactory";
...
const AiFinancialAdviceFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = aiFinancialAdviceFakerFactory(props.user,fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("aiFinancialAdvice").create(elem));
...
export default connect(mapState, mapDispatch)(AiFinancialAdviceFakerDialogComponent);
